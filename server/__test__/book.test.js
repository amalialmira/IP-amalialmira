...
describe ('GET /books', () => {
    test('GET /books success get all books without filter parameter returning all books', async () => {
        ...
        expect(response.body.data[0]).toHaveProperty('publisher', expect.any(String));
    ...
    test('GET /books Success get books with filter parameter returning filtered books', async () => {
        let response = await request(app).get('/books?filter=fantasy')
    ...
    test('GET /books Success get products with pagination parameter returning paginated books', async () => {
        let response = await request(app).get('/books?page[size]=5&page[number]=2')
        expect(response.status).toBe(200)
        expect(response.body).toBeInstanceOf(Object)
        expect(response.body).toHaveProperty('page', expect.any(Number))
        expect(response.body).toHaveProperty('data', expect.any(Array))
